            ...
            <header className="sticky top-0 z-50 border-b border-slate-800 bg-[#0a0d1f]/90 backdrop-blur-md px-6 lg:px-10 py-3.5">
                <div className="max-w-[1400px] mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
                        <div className="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center shadow-[0_0_15px_rgba(60,131,246,0.4)] group-hover:shadow-[0_0_25px_rgba(60,131,246,0.6)] transition-all">
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        {['Dashboard', 'Projects', 'Models', 'Settings'].map((item, i) => (
                            <a key={item} className={`text-sm font-medium transition-colors ${i === 1 ? 'text-blue-400' : 'text-slate-400 hover:text-white'}`} href="#">{item}</a>
                        ))}
                    </nav>
                    <div className="flex items-center gap-3">
                        <button className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-slate-800 transition-colors">
                            <span className="material-symbols-outlined text-slate-400 text-xl">notifications</span>
                        </button>
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-sm font-bold cursor-pointer">SY</div>
                    </div>
                </div>
            </header>
